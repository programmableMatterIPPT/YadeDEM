...
class PmCohFrictMat : public FrictMat
{
	public :
		virtual shared_ptr<State> newAssocState() const { return shared_ptr<State>(new PmState); }
		virtual bool stateTypeOk(State* s) const { return (bool)dynamic_cast<PmState*>(s); }
		virtual ~PmCohFrictMat () {};
/// Serialization
	YADE_CLASS_BASE_DOC_ATTRS_CTOR(PmCohFrictMat,FrictMat,"",
		...
		((Real,normalCohesion1,0,,"")) //added
		((Real,shearCohesion1,0,,"")) //added
		((Real,normalCohesion2,0,,"")) //added
		((Real,shearCohesion2,0,,"")) //added
		((Real,normalCohesion3,0,,"")) //added
		((Real,shearCohesion3,0,,"")) //added
		((Real,normalCohesion4,0,,"")) //added
		((Real,shearCohesion4,0,,"")) //added
		((int,num,0,,"explicit material number for use in cross-cohesion")) //added
		((Real,cn,0,,"normal viscous damping coefficient")) //added
		((Real,cs,0,,"shear viscous damping coefficient")) //added
...
/// Indexable
	REGISTER_CLASS_INDEX(PmCohFrictMat,FrictMat);
};
REGISTER_SERIALIZABLE(PmCohFrictMat);


class PmCohFrictPhys : public FrictPhys
{
	public :
		virtual ~PmCohFrictPhys() {};
		void SetBreakingState() {cohesionBroken = true; normalAdhesion = 0; shearAdhesion = 0;};

	YADE_CLASS_BASE_DOC_ATTRS_CTOR(PmCohFrictPhys,FrictPhys,"",
		...
		((Real,cn,0,,"normal viscous damping coefficient")) //added
		((Real,cs,0,,"shear viscous damping coefficient")) //added
		((Real,cr,0,,"roll viscous damping coefficient")) //added
		((Real,ctw,0,,"twist viscous damping coefficient")) //added
		,
		createIndex();
	);
/// Indexable
	REGISTER_CLASS_INDEX(PmCohFrictPhys,FrictPhys);
	DECLARE_LOGGER;	 //added
};
REGISTER_SERIALIZABLE(PmCohFrictPhys);


class PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment: public LawFunctor{
	public:
	...
	YADE_CLASS_BASE_DOC_ATTRS_CTOR_PY(PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment,LawFunctor,"Law for linear traction-compression-bending-twisting, with cohesion+friction and Mohr-Coulomb plasticity surface. This law adds adhesion and moments to :yref:`Law2_ScGeom_FrictPhys_CundallStrack`.\n\nThe normal force is (with the convention of positive tensile forces) $F_n=min(k_n*u_n, a_n)$, with $a_n$ the normal adhesion. The shear force is $F_s=k_s*u_s$, the plasticity condition defines the maximum value of the shear force, by default $F_s^{max}=F_n*tan(\\phi)+a_s$, with $\\phi$ the friction angle and $a_s$ the shear adhesion. If :yref:`CohFrictPhys::cohesionDisableFriction` is True, friction is ignored as long as adhesion is active, and the maximum shear force is only $F_s^{max}=a_s$.\n\nIf the maximum tensile or maximum shear force is reached and :yref:`CohFrictPhys::fragile` =True (default), the cohesive link is broken, and $a_n, a_s$ are set back to zero. If a tensile force is present, the contact is lost, else the shear strength is $F_s^{max}=F_n*tan(\\phi)$. If :yref:`CohFrictPhys::fragile` =False (in course of implementation), the behaviour is perfectly plastic, and the shear strength is kept constant.\n\nIf :yref:`Law2_ScGeom6D_CohFrictPhys_CohesionMoment::momentRotationLaw` =True, bending and twisting moments are computed using a linear law with moduli respectively $k_t$ and $k_r$ (the two values are the same currently), so that the moments are : $M_b=k_b*\\Theta_b$ and $M_t=k_t*\\Theta_t$, with $\\Theta_{b,t}$ the relative rotations between interacting bodies. The maximum value of moments can be defined and takes the form of rolling friction. Cohesive -type moment may also be included in the future.\n\nCreep at contact is implemented in this law, as defined in [Hassan2010]_. If activated, there is a viscous behaviour of the shear and twisting components, and the evolution of the elastic parts of shear displacement and relative twist is given by $du_{s,e}/dt=-F_s/\\nu_s$ and $d\\Theta_{t,e}/dt=-M_t/\\nu_t$.",
		...
		((bool,shearBreaksCohesion,true,,"should cohesion break when only shear adhesion is exceeded, but not normal adhesion"))//added
		,,
		.def("normElastEnergy",&PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment::normElastEnergy,"Compute normal elastic energy.")
		.def("shearElastEnergy",&PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment::shearElastEnergy,"Compute shear elastic energy.")
		.def("bendingElastEnergy",&PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment::bendingElastEnergy,"Compute bending elastic energy.")
		.def("twistElastEnergy",&PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment::twistElastEnergy,"Compute twist elastic energy.")
		.def("elasticEnergy",&PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment::totalElastEnergy,"Compute total elastic energy.")
	);
	FUNCTOR2D(ScGeom6D,PmCohFrictPhys);
	DECLARE_LOGGER;
};
REGISTER_SERIALIZABLE(PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment);


class PmCohesiveFrictionalContactLaw : public GlobalEngine
{
	shared_ptr<PmLaw2_ScGeom6D_CohFrictPhys_CohesionMoment> functor;
	
	public :		
		long iter;/// used for checking if new iteration
		void action();
		
	YADE_CLASS_BASE_DOC_ATTRS(PmCohesiveFrictionalContactLaw,GlobalEngine,"[DEPRECATED] Loop over interactions applying :yref:`Law2_ScGeom6D_CohFrictPhys_CohesionMoment` on all interactions.\n\n.. note::\n  Use :yref:`InteractionLoop` and :yref:`Law2_ScGeom6D_CohFrictPhys_CohesionMoment` instead of this class for performance reasons.",
		((bool,neverErase,false,,"Keep interactions even if particles go away from each other (only in case another constitutive law is in the scene, e.g. :yref:`Law2_ScGeom_CapillaryPhys_Capillarity`)"))
		((bool,always_use_moment_law,false,,"If true, use bending/twisting moments at all contacts. If false, compute moments only for cohesive contacts."))
		((bool,shear_creep,false,,"activate creep on the shear force, using :yref:`CohesiveFrictionalContactLaw::creep_viscosity`."))
		((bool,twist_creep,false,,"activate creep on the twisting moment, using :yref:`CohesiveFrictionalContactLaw::creep_viscosity`."))
...
REGISTER_SERIALIZABLE(PmCohesiveFrictionalContactLaw);


class PmIp2_CohFrictMat_CohFrictMat_CohFrictPhys : public IPhysFunctor
{
	public :
		...
		YADE_CLASS_BASE_DOC_ATTRS_CTOR(PmIp2_CohFrictMat_CohFrictMat_CohFrictPhys,IPhysFunctor,
	...
	FUNCTOR2D(PmCohFrictMat,PmCohFrictMat);
};
REGISTER_SERIALIZABLE(PmIp2_CohFrictMat_CohFrictMat_CohFrictPhys);
